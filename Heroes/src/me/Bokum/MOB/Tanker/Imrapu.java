...
public class Imrapu extends Ability{
	...
	public void PrimarySkill(final Player p){
		if(Cooldown.Checkcooldown(p, "주스킬")){
			...
			final LivingEntity t = MobSystem.Getcorsurplayer(p, can_passive ? 20 : 15);
			if(t == null){
			...
			le.start();
			if(t instanceof Player) MobSystem.CancelSkill((Player) t, 3);
		...
		if(Cooldown.Checkcooldown(p, "보조스킬")){
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(Main.instance, new Runnable(){
				public void run(){
					...
					ParticleEffect.VILLAGER_HAPPY.display(0.3f, 0.3f, 0.3f, 0.1f, 15, p.getLocation(), 20);
					cc = false;
				}
			}, 60l);
		}
	...
	public void UltimateSkill(final Player p){
		if(Cooldown.Checkcooldown(p, "궁극기")){
			...
			Cooldown.Setcooldown(p, "궁극기", 128, true);
	...
	public void Ultimate(final Location l){
		...
		timernum1 = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.instance, new Runnable(){
			public void run(){
				if(--timertime1 > 0){
					for(Entity entity : imrapu.getNearbyEntities(11, 11, 11)){
