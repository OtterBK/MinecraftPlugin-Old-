...
public class Engineer extends Ability{
	...
	public void description(){
		engineer.sendMessage("§6============= §f[ §b엔지니어 §f] - §e직업군 §7: §cExpert §6=============\n"
				...
				+ "§f- §2패시브§f(§c구매시 사용가능§f) §7: §d파워 건전지\n§7대나무 헬리콥터의 지속시간이 1.5초, 수퍼장갑의 지속시간이 3초 증가합니다.");
	}
	...
	public void PrimarySkill(final Player p){
		if(Cooldown.Checkcooldown(p, "주스킬")){
			if(Main.Bluetop.distance(p.getLocation()) <= 50 || Main.Redtop.distance(p.getLocation()) <= 50){
				p.sendMessage(Main.MS+"포탑 근처 50칸내에서는 사용하실 수 없습니다.");
				return;
			}
			...
			p.setFlying(true);
			Cooldown.SkillCountdown(p, can_passive ? 5 : 3, "소형 헬리콥터 사용");
	...
	public void UltimateSkill(final Player p){
		if(Cooldown.Checkcooldown(p, "궁극기")){
			...
			if(b.getLocation().distance(Main.Bluetop) <= 20 || b.getLocation().distance(Main.Redtop) <= 20){
				p.sendMessage(Main.MS+"포탑 주변 20칸내를 목표로는 사용하실 수 없습니다.");
				return;
			}
			Cooldown.Setcooldown(p, "궁극기", 53, true);
