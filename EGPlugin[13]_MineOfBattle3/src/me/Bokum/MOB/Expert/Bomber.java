...
public class Bomber extends Ability{
	...
	public void PrimarySkill(final Player p){
		if(Cooldown.Checkcooldown(p, "주스킬")){
			if(p.getLocation().getBlock().getType() != Material.AIR || p.getLocation().getBlock().getRelative(0, -1, 0).getType() == Material.STATIONARY_WATER
					|| p.getLocation().getBlock().getRelative(0, -1, 0).getType() == Material.WATER){
