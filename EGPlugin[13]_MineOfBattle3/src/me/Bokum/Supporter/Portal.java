...
public class Portal extends Ability{
	...
	public void PrimarySkill(final Player p){
		if(Cooldown.Checkcooldown(p, "주스킬")){
			...
			if(b.getRelative(0,1,0).getType() != Material.AIR){
				p.sendMessage(Main.MS+"여기에는 설치하실 수 없습니다.");
				return;
			}
			if(b.getRelative(0,1,0).getLocation().getBlockY() >= 26){
				p.sendMessage(Main.MS+"Y26 이상에는 설치하실 수 없습니다.");
				return;
			}
			if((b.getRelative(0, 1, 0).getType() != Material.AIR && b.getRelative(0, 1, 0).getType() != Material.PORTAL ) 
				...
				p.sendMessage(Main.MS+"대상 블럭의 3칸 위에 아무블럭이 없어야합니다.");
				return;
			}
			for(int i = 0; i < 3; i++){
				if(b.getRelative(0, -1, 0).getLocation().distance(Main.Core[i]) <= 45){
					p.sendMessage(Main.MS+"점령지의 45칸내에는 포탈을 설치하실 수 없습니다.");
					return;
				}
			}
			if(Main.Bluetop.distance(p.getLocation()) <= 35 || Main.Redtop.distance(p.getLocation()) <= 45){
				p.sendMessage(Main.MS+"포탑 근처 45칸내에서는 설치하실 수 없습니다.");
				return;
			}
			if(Main.Bluespawn.distance(p.getLocation()) <= 35 || Main.Redspawn.distance(p.getLocation()) <= 35){
				p.sendMessage(Main.MS+"기지 근처 35칸내에서는 설치하실 수 없습니다.");
				return;
			}
			if(Main.Bluedeath.distance(p.getLocation()) <= 15 || Main.Redspawn.distance(p.getLocation()) <= 15){
				p.sendMessage(Main.MS+"리스폰 대기 지점 15칸내에서는 설치하실 수 없습니다.");
				return;
			}
			...
			blueportal.getBlock().getRelative(0, -1, 0).setTypeIdAndData(35, (byte)(MobSystem.bluelist.contains(p) ? 11 : 14), true);
	...
	public void SecondarySkill(final Player p){
		if(Cooldown.Checkcooldown(p, "보조스킬")){
			...
			for(int i = 0; i < 3; i++){
				if(b.getRelative(0, -1, 0).getLocation().distance(Main.Core[i]) <= 35){
					p.sendMessage(Main.MS+"점령지의 35칸내에는 포탈을 설치하실 수 없습니다.");
					return;
				}
			}
			if(Main.Bluetop.distance(p.getLocation()) <= 30 || Main.Redtop.distance(p.getLocation()) <= 30){
				p.sendMessage(Main.MS+"포탑 근처 30칸내에서는 사용하실 수 없습니다.");
				return;
			}
			if(Main.Bluespawn.distance(p.getLocation()) <= 35 || Main.Redspawn.distance(p.getLocation()) <= 35){
				p.sendMessage(Main.MS+"기지 근처 35칸내에서는 설치하실 수 없습니다.");
			...
			redportal = new Location(b.getWorld(), b.getLocation().getBlockX(), b.getLocation().getBlockY()+2
					, b.getLocation().getBlockZ());
			redportal.getBlock().getRelative(0, -1, 0).setTypeIdAndData(35, (byte)(MobSystem.bluelist.contains(p) ? 11 : 14), true);
	...
	public void thirdSkill(final Player p){
		if(Cooldown.Checkcooldown(p, "보조스킬2")){
			Player t = MobSystem.Getcorsurplayer(p, 25);
			if(t == null || MobSystem.getTeam(p).contains(t) || t.getName().contains("[") || t.getName().contains("]")){
				p.sendMessage(Main.MS+"25칸내에 바라보는곳에 적이 없습니다.");
				return;
			}
		    ...
		    t.sendMessage(Main.MS+"포탈의 스킬에 의해 "+portal.getName()+" 님과 위치가 바뀌었습니다.");
		    		...
		    		if(--timertime < 0){
			    		...
			    		for(Player t : MobSystem.getEnemy(p)){
			    			if(t.getLocation().distance(loc) <= 3){
			    				...
			    				t.playSound(t.getLocation(), Sound.PORTAL, 2.0f, 0.5f);
			    				t.sendMessage(Main.MS+"포탈의 차원 주머니에 빨려 들어가버렸습니다.");
			    			}
			    		}
		    		}
		    	}
