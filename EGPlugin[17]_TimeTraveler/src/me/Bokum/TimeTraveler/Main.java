...
public class Main extends JavaPlugin implements Listener{
	...
	public static boolean inGameCheck = true;
	...
	public void onEnable(){
		...
		getLogger().info("시간 여행자들 플러그인이 로드 되었습니다.");
	}
	...
	public void onDisable(){
		getLogger().info("게임제목 플러그인이 언로드 되었습니다.");
	}
	...
	public static void gameJoin(Player p) {
		...
		if(check_Start) {
			...
			me.Bokum.EGM.Main.gamelist.put(p.getName(), "게임제목");
			Sendmessage(MS+p.getName()+" 님이 게임제목에 참여하셨습니다. 인원 (§e "+plist.size()+"§7 / §c6 §f)");
	...
	public static void startGame() {
		...
		tasktime[cur] = 5;
		Bukkit.broadcastMessage(MS+"§b§l게임제목§f가 곧 시작됩니다.");
