...
public class FlameWizard extends Ability{
	...
	public FlameWizard(String playername, Player p){
		...
		flameWizard = p;
		ItemStack item = new ItemStack(377, 1);
	...
	public void description(){
		flameWizard.sendMessage("§6============= §f[ §b플레임 위자드 §f] - §e직업군 §7: §cAttacker §6=============\n"
				...
				+ "§f- §a보조스킬 §7: §d플레임 워\n§77칸내에 있는 적에게 7의 데미지를 주고 12초간 화상상태로 만듭니다.\n"
			...
			timernum = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.instance, new Runnable(){
				public void run(){
					if(timertime-- > 0){
						...
						fireball.setIsIncendiary(false);
						fireball.setFireTicks(can_passive ? 160 : 80);
						fireball.setDirection(new Vector(0, -1.5f, 0));
					}else{
	...
	public void onHit(EntityDamageByEntityEvent e){
		...
		if(e.getDamager() instanceof Fireball){
			...
			e.setCancelled(true);
			if(MobSystem.getTeam(d).contains(p)){
				return;
			}
		...
		}else if(flameWizard.getInventory().getHeldItemSlot() == 0){{
			try{
